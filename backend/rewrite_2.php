<?php                     
   ...
   $query = "SELECT  * FROM `member` WHERE `account`= '$account' AND `plate`='$oldaddress'"; 
    $result = mysqli_query($conDb, $query);
    $count=mysqli_num_rows($result);
    if($count>0&& $newaddress!=$oldaddress)
    {
        $tmp= "UPDATE `member` SET `plate` = '$newaddress' where `member`.`account` = '$account'";  
